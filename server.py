...
def newItem():
    if 'username' not in login_session:
        ...
        if request.method == 'POST':
            ...
            print category
            newItem = Item(name = name, description = description, category_id = int(category) )
            session.add(newItem)
            session.commit()
            return redirect(url_for('Home'))
        else:
...
def categoryView(category):
    ...
    if 'username' not in login_session:
        ...
        return render_template('category.html', login = True, items = items, categories = categories)
...
def itemView(category, item):
    ...
    if 'username' not in login_session:
        ...
        return render_template('item.html', login = True, item = item, category = category)
...
def editItem(category, item):
    if 'username' not in login_session:
        ...
        cat_id = session.query(Category).filter_by(name= category).one().id
        edit_item = session.query(Item).filter_by(category_id = cat_id, name = item).one()
        if request.method == 'POST':
            name = request.form['name']
            description = request.form['description']
            category = request.form['category']
            ...
            categories = session.query(Category).all()
            return render_template('edit_item.html', item = edit_item, categories = categories)
...
def deleteItem(category, item):
    if 'username' not in login_session:
        ...
        if request.method == 'POST':
            cat_id = session.query(Category).filter_by(name= category).one().id
            item_del =  session.query(Item).filer_by(name = item, category_id = cat_id).delete()
            return redirect(url_for('Home'))
